...
exports.addFee = (req, res) => {
    ...
    db.query(getRoom, [student_id], (err, result) => {
        ...
        // Step 2: Insert fee
        const sql = "INSERT INTO fees (student_id, amount, status) VALUES (?, ?, 'Unpaid')";
...
// AFTER ✅ - shows ALL students, missing fee shows as Unpaid
exports.getFees = (req, res) => {
    const sql = `
        SELECT 
            ...
            students.name,
            COALESCE(fees.amount, rooms.fee) AS amount,
            COALESCE(fees.status, 'Unpaid') AS status
        FROM students
        JOIN rooms ON students.room_id = rooms.id
...
exports.updateFee = (req, res) => {
    ...
    // First, get the room fee amount for this student
    const getAmount = `
        SELECT rooms.fee 
        FROM students
        JOIN rooms ON students.room_id = rooms.id
    ...
    db.query(getAmount, [student_id], (err, result) => {
        if (err) return res.send("Failed to fetch student");
...
exports.getFeeByStudent = (req, res) => {
    ...
    const sql = `
        SELECT students.name, rooms.fee, rooms.room_number, fees.status
        FROM students
        JOIN rooms ON students.room_id = rooms.id
        LEFT JOIN fees ON fees.student_id = students.id
        WHERE students.id = ?
    ...
    db.query(sql, [student_id], (err, result) => {
        ...
        if (result.length === 0) {
            return res.json({ name: "", fee: 0, room_no: "Not Assigned", status: "Unpaid" });
        }

        res.json({
            name: result[0].name,
            fee: result[0].fee,
            room_no: result[0].room_no,   // ✅ added
            status: result[0].status || "Unpaid"
        });
    });
};